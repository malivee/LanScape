...
final class MotionDetectionService: ObservableObject {
    ...
    private let motionQueue = DispatchQueue(label: "motionDetectionQueue", qos: .userInitiated)
    ...
    func processPixelBuffer(_ pixelBuffer: CVPixelBuffer) {
        guard isTrackingActive, !isCompleted, !isComputing else { return }
        ...
        motionQueue.async { [weak self] in
            ...
            guard let baseAddress = CVPixelBufferGetBaseAddress(pixelBuffer) else {
                Task { @MainActor in self.isComputing = false }
                return
            }
            ...
            currentGrid.reserveCapacity(self.gridWidth * self.gridHeight)
            
            let stepX = max(1, width / self.gridWidth)
            let stepY = max(1, height / self.gridHeight)
            let buffer = baseAddress.assumingMemoryBound(to: UInt8.self)
            
            for yIndex in 0..<self.gridHeight {
                let rowOffset = (yIndex * stepY) * bytesPerRow
                for xIndex in 0..<self.gridWidth {
            ...
            guard !self.previousGrid.isEmpty, self.previousGrid.count == currentGrid.count else {
                self.previousGrid = currentGrid
                Task { @MainActor in self.isComputing = false }
                return
            }
            ...
            for i in 0..<currentGrid.count {
                diffSum += abs(Int(currentGrid[i]) - Int(self.previousGrid[i]))
            }
