...
final class VisionService: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func startSession() {
        sessionQueue.async { [weak self] in
            ...
            DispatchQueue.main.async {
                self.debugStatus = "Camera running"
            }
        }
    }
    ...
    func stopSession() {
        sessionQueue.async { [weak self] in
            ...
            DispatchQueue.main.async {
                self.debugStatus = "Camera stopped"
            }
        }
    }
    ...
    private func checkCameraPermission() {
        ...
        case .denied:
            DispatchQueue.main.async {
                self.debugStatus = "Camera permission denied"
            }

        case .restricted:
            DispatchQueue.main.async {
                self.debugStatus = "Camera restricted"
            }
    ...
    private func setupSession() {
        sessionQueue.async { [weak self] in
            ...
            guard
                ...
                print("❌ Could not configure FRONT camera")
    ...
    private func currentOrientationAndSize() -> (CGImagePropertyOrientation, CGSize) {
        return (.up, CGSize(width: 1920, height: 1080))
    }
        ...
        do {
            ...
            guard
                ...
                DispatchQueue.main.async { [weak self] in
                    ...
                    self?.detectedPlayerCount = 0
                    self?.debugStatus = "No person detected"
                }
                return
            }

            // Tracked joints: 2 hands (left/right wrist) + 2 legs (left/right ankle)
            ...
            // Reverse X sorting for front-camera preview mirroring
            // (Leftmost person on screen -> Player 1; Rightmost person -> Player 2)
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self?.detectedPlayerCount = people.count
                self?.debugStatus = "Tracking \(people.count) player(s)"
            }

        } catch {
            DispatchQueue.main.async { [weak self] in
                self?.debugStatus = "Vision error: \(error.localizedDescription)"
            }
        }
    }
}
